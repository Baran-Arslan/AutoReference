...
namespace iCare.AutoReference.Editor {
    internal sealed class DependencyResolver {
        ...
        object ResolveData(ReferenceAttribute attribute, Type fieldElementType) {
            object result;
            switch (attribute) {
                case AutoAttribute autoAttribute:
                    result = ResolveAutoAttribute(fieldElementType, autoAttribute.ID);
                    break;
                case AutoListAttribute autoListAttribute:
                    result = ResolveAutoListAttribute(fieldElementType, autoListAttribute.ID);
                    break;
                default:
                    throw new ArgumentOutOfRangeException(nameof(attribute), "Unsupported attribute type");
            }
